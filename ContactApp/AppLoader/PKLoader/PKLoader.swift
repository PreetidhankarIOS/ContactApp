...
//  PKLoader.swift
//
//  Created by Pawan Kumar on 06/09/18.
    ...
    private var parantView: UIView? {
        didSet {
            if let parent = self.parantView {
                ...
                self.parentPreviousCornerRadius = parent.layer.cornerRadius
                self.parentPreviousBorderWidth = parent.layer.borderWidth
                self.parentPreviousBorderColor = parent.layer.borderColor ?? UIColor.clear.cgColor
            }
            else {
                self.parentPreviousRect = CGRect.zero
                self.parentPreviousCornerRadius = 0
                self.parentPreviousBorderWidth = 0
                self.parentPreviousBorderColor = UIColor.clear.cgColor
            }
        }
    }
