...
// Enum that used to create diffrent type of shapes used in creating animations
enum PKIndicatorShape {
    case circle
    case circleSemi
    case ring
    ...
    func layerWith(size: CGSize, color: UIColor) -> CALayer {
        ...
        case .ringTwoHalfVertical:
            path.addArc(withCenter: CGPoint(x: size.width / 2, y: size.height / 2),
                        ...
                        clockwise: true)
            path.move(to: CGPoint(x: size.width / 2 - size.width / 2 * cos(CGFloat(Double.pi / 4)),
